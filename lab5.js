...
const Lab5 = (app) => {
  ...
  app.get("/a5/todos/:id/title/:title", (req, res) => {
    ...
    if (!todo) {
      res.res
        .status(404)
        .json({ message:
          `Unable to update title Todo with ID ${id}` });
      return;
    }
    todo.title = title;
    res.json(todos);
  ...
  app.post("/a5/todos/:id/title/:title", (req, res) => {
    const { id, title } = req.params;
    const todo = todos.find((t) => t.id === parseInt(id));
    todo.title = title;
    res.json(todos);
  ...
  app.get("/a5/assignment", (req, res) => {
    res.json(assignment); //json is the same as send but makes the data a json type. same like toString
  });
  ...
  app.get("/a5/calculator", (req, res) => {
    const { a, b, operation } = req.query; // gets the parameters and parse them into a string
    let result = 0;
    switch (operation) {
      case "add":
